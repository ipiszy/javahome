...
public class Analyzer {
    ...
    public static String byteToStr(byte[] byteData) {
        ...
        if (byteData.length % 16 == 0) {
            level = byteData.length / 16 - 1;
        } else {
            level = byteData.length / 16;
        }
        for (int r = 0; r <= level; r++) {
            ...
            for (int c = 0; c < 16; c++) {
                ...
                if (o >= byteData.length) {
                    ...
                    if (strByte[c] == 0 || strByte[c] == 10) {
                        strByte[c] = 46; // Show 00 as .
                    }
                    result = result + byteToHex(byteData[r * 16 + c], 2) + " ";
                }
            }
    ...
    public static String getEthernetProtocol(int protocol) {
        switch (protocol) {
            ...
            case 0x0001:
                return "Dummy type for 802.3 frames";
            case 0x0002:
                return "Dummy protocol id for AX.25";
            case 0x0003:
                return "Every packet";
            case 0x0004:
                return "802.2 frames";
            ...
            case 0x0006:
                return "DEC DDCMP: Internal only";
            case 0x0007:
                return "Dummy type for WAN PPP frames";
            case 0x0008:
                return "Dummy type for PPP MP frames";
            case 0x0009:
                return "Localtalk pseudo type";
            case 0x0010:
                return "Dummy type for Atalk over PPP";
            case 0x0011:
                return "802.2 frames";
            case 0x0015:
                return "Mobitex (kaz@cafe.net)";
            case 0x0016:
                return "Card specific control frames";
            case 0x0017:
                return "Linux/IR";
            case 0x0026:
                return "spanning tree bridge protocol";
            case 0x886d:
                return "intel adapter fault tolerance heartbeats";
            case 0xffff:
                return "Ethernet protocol mask";
            default:
                return "Unknown";
        }
    }
    ...
    public static String getTypeOfService(int typeOfService) {
        String result = "";
        if ((typeOfService & 0x10) > 0) {
            result = result + "Minimize_Delay ";
        }
        if ((typeOfService & 0x08) > 0) {
            result = result + "Maximize_Throughput ";
        }
        if ((typeOfService & 0x04) > 0) {
            result = result + "Maximize_Reliability ";
        }
        if ((typeOfService & 0x02) > 0) {
            result = result + "Minimize_Monetary_Cost ";
        }
        if (result.equals("") || (typeOfService & 0x01) > 0) {
    ...
    public static String getARPOperation(int operation) {
        if (operation == ARPPacket.ARP_OP_REP_CODE) {
            return "ARP Reply";
        }
        return "ARP Request";
    }
